...
async function loadCountriesData() {
    try {
        const countries: Country[] = await getData('https://restcountries.com/v3.1/all?fields=name&fields=cca3&fields=area');
...
(async () => {
    ...
    (document.getElementById('form') as HTMLFormElement).addEventListener('submit', async (event) => {
        ...
        const visitedFrom: Record<string, string | null> = { [cca3CountryFrom]: null };
        ...
        if (!visitedFrom[cca3CountryTo]) {
            ...
            const route = [];
            let currentCountry: string | null = cca3CountryTo;
